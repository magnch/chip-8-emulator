...
pub fn main() {
    ...
    let mut audio_player =
        audio::AudioPlayer::new(&sdl_context).expect("failed to init audio player");
    ...
    'running: loop {
            ...
            match event {
                ...
                Event::KeyDown {
                    ...
                    if let Some(chip8_key) = input::map_scancode(sc) {
                        ...
                        chip8
                            .key_down(chip8_key)
                            .expect("map_scancode returns a valid CHIP-8 key");
                    }
                }
                Event::KeyUp {
                    scancode: Some(sc), ..
        ...
        ::std::thread::sleep(
            Duration::min(cpu_step_duration, timer_step_duration).saturating_sub(elapsed),
        );
    }
}
